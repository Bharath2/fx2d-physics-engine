...
class FxVec2f : public Eigen::Vector2f {
  public:
    ...
    FxVec2f rotate(float theta) const noexcept { return FxVec2f(*this).rotate_inplace(theta); }
    ...
    float cross(const FxVec2f& other) const { return x() * other.y() - y() * other.x(); }
...
class FxVec2d : public Eigen::Vector2d {
  public:
    ...
    // — non-mutating rotation: returns a rotated copy
    FxVec2d rotate(double theta) const noexcept { return FxVec2d(*this).rotate_inplace(theta); }
    FxVec2d rotate_rad(double theta) const noexcept {
        return FxVec2d(*this).rotate_inplace_rad(theta);
    }

    // Cross product with another 2D vector (returns scalar)
    double cross(const FxVec2d& other) const { return x() * other.y() - y() * other.x(); }
...
template<Numeric S>
inline FxVec2f operator*(FxVec2f const& v, S s) {
    return FxVec2f(v.array() * static_cast<float>(s));
}
template<Numeric S>
...
template<Numeric S>
inline FxVec3f operator*(FxVec3f const& v, S s) {
    return FxVec3f(v.array() * static_cast<float>(s));
}
template<Numeric S>
...
template<Numeric S>
inline FxVec2d operator*(FxVec2d const& v, S s) {
    return FxVec2d(v.array() * static_cast<double>(s));
}
template<Numeric S>
inline FxVec2d operator*(S s, FxVec2d const& v) {
    return v * s;
}
template<Numeric S>
inline FxVec2d operator/(FxVec2d const& v, S s) {
...
template<Numeric S>
inline FxVec2d operator-(S s, FxVec2d const& v) {
    return FxVec2d((FxVec2d::Scalar(static_cast<double>(s)) * FxVec2d::Ones()).array() - v.array());
}

// FxVec3d scalar operations with generic Numeric s
template<Numeric S>
inline FxVec3d operator*(FxVec3d const& v, S s) {
    return FxVec3d(v.array() * static_cast<double>(s));
}
template<Numeric S>
inline FxVec3d operator*(S s, FxVec3d const& v) {
    return v * s;
}
template<Numeric S>
inline FxVec3d operator/(FxVec3d const& v, S s) {
...
template<Numeric S>
inline FxVec3d operator-(S s, FxVec3d const& v) {
    return FxVec3d((FxVec3d::Scalar(static_cast<double>(s)) * FxVec3d::Ones()).array() - v.array());
}

// FxVec4f scalar operations with generic Numeric s
template<Numeric S>
inline FxVec4f operator*(FxVec4f const& v, S s) {
    return FxVec4f(v.array() * static_cast<float>(s));
}
template<Numeric S>
...
class FxMat2f : public Eigen::Matrix2f {
  public:
    ...
    FxMat2f inv_rotation() const { return this->transpose(); }
};
...
concept NumericOrFxVec =
    ...
    || std::same_as<T, FxVec2f> || std::same_as<T, FxVec3f> || std::same_as<T, FxVec4f>;
// || std::same_as<T, FxVecXf>;

template<typename T>
concept FxVecT = std::same_as<T, FxVec2f> || std::same_as<T, FxVec3f> || std::same_as<T, FxVec4f>;
...
struct FxArrayAlignedDelete {
    std::size_t align;
    void operator()(T* p) const noexcept { ::operator delete[](p, std::align_val_t(align)); }
};

// Helper function to create aligned arrays
template<typename T>
static std::unique_ptr<T[], FxArrayAlignedDelete<T>> FxArray_make_aligned(std::size_t n,
                                                                          std::size_t align = 32) {
...
class FxArray {
  ...
  protected:
    ...
    std::size_t checkIndex(I idx) const {
        ...
        if (i < 0 || u >= m_size) {
            throw std::out_of_range("FxArray::[] index " + std::to_string(i) +
  ...
  public:
    ...
    explicit FxArray(std::size_t n) : m_size(n), m_arr(FxArray_make_aligned<T>(n, kAlign)) {}

    // 3) Dedicated init_list ctor — for braced lists
    FxArray(std::initializer_list<T> init) : FxArray(init.size()) {
        std::copy(init.begin(), init.end(), aligned_data());
    }

    // 4) one ctor for std::vector
    FxArray(std::vector<T> const& v) : FxArray(v.size()) {
        std::copy(v.begin(), v.end(), aligned_data());
    }

    // 5) one overload for c style array
    template<std::size_t N>
    FxArray(T const (&arr)[N]) : FxArray(N) {
        std::copy_n(arr, N, aligned_data());
    }

    // deep‐copy copy‐ctor using copy_n
    FxArray(FxArray const& o) : FxArray(o.m_size) {
    ...
    T& operator()(I i) noexcept {
        return aligned_data()[static_cast<size_t>(i)];
    }
    ...
    template<std::integral I>
    T& at(I idx) {
        return aligned_data()[checkIndex(idx)];
    }
    template<std::integral I>
    ...
    template<typename U>
        requires ConvertibleOrNumeric<U, T>
    ...
    FxArray& rotate_inplace_rad(float theta_rad) noexcept
        requires std::same_as<T, FxVec2f>
    {
...
template<typename T, typename U>
    requires ConvertibleOrNumeric<U, T>
inline FxArray<T> operator+(FxArray<T> a, U const& scalar) {
    return a += scalar;
}
...
template<typename T, typename U>
    requires ConvertibleOrNumeric<U, T>
inline FxArray<T> operator+(U const& scalar, FxArray<T> a) {
    return a += scalar;
}
...
// Array–Array
template<typename T>
inline FxArray<T> operator+(FxArray<T> a, const FxArray<T>& b) {
    return a += b;
}
