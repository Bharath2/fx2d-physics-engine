...
const char* phase_name(Phase p) {
    switch (p) {
        case Phase::Position: return "POSITION  (arm -> target angle, slider -> target offset)";
        case Phase::Velocity: return "VELOCITY  (arm spins at 3.14 rad/s, slider at 1.0 m/s)";
        case Phase::Effort:   return "EFFORT    (raw torque 15 N*m, raw force -6 N)";
    }
    return "";
}
...
int main() {
    ...
    std::cout << "Joint control demo — each control mode runs for "
              << PHASE_SECONDS << " s, then the cycle repeats.\n"
